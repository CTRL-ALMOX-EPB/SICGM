...
// ============================================
// FUNÇÕES PARA A NOVA FUNCIONALIDADE
// ============================================
...
function consolidarPendenciasNovo(devolucaoItens, movimentosSiago, movimentosBanco) {
    console.log('🔄 Consolidando pendências...');
    ...
    console.log(`✅ ${pendencias.length} pendências consolidadas`);
    return pendencias;
}

async function carregarDadosCompletosNovo() {
    console.log('🚀 Iniciando carregamento completo da nova visão...');
    ...
    try {
        ...
        console.log(`📊 Dados carregados (nova visão):`);
        ...
        // Esconder o loading da nova visão
        const loadingElement = document.getElementById('loadingNovo');
        if (loadingElement) loadingElement.style.display = 'none';
        
        // Atualizar o dashboard com a nova visão
        atualizarDashboardNovo();
        
        return true;
        
    } catch (error) {
        console.error('❌ Erro ao carregar dados (nova visão):', error);
        const loadingElement = document.getElementById('loadingNovo');
        if (loadingElement) {
            loadingElement.innerHTML = `
                <div style="color: #FC8181; text-align: center; padding: 20px;">
                    <div style="font-size: 36px;">❌</div>
                    <p style="margin-top: 10px;">Erro ao carregar dados</p>
...
function atualizarDashboardNovo() {
    console.log('🔄 Atualizando dashboard (nova visão)...');
    
    const containerNovo = document.getElementById('dashboardNovo');
    if (!containerNovo) {
        console.warn('⚠️ Container da nova visão não encontrado');
        return;
    }
    ...
    console.log(`✅ Dashboard atualizado com ${dadosExibir.length} registros (nova visão)`);
}
    ...
    for (const obra in gruposPorObra) {
        ...
        if (isActive) {
            grupo.itens.forEach(item => {
                ...
                const qtdInfo = item.status === 'ATENDIDO' 
                    ? `${item.qtdEncontrada.toFixed(2)}/${item.qtdEsperada.toFixed(2)}`
                    : `${item.qtdEncontrada.toFixed(2)}/${item.qtdEsperada.toFixed(2)}`;
...
function trocarAbaNovo(aba) {
    console.log(`🔄 Trocando para aba: ${aba} (nova visão)`);
    abaAtualNovo = aba;
    itemSelecionadoNovo = null;
    ...
    if (listTitle) {
        const titles = {
            'mgm': '📌 Pendências MGM',
            'separacao': '📦 Pendências de Separação'
        };
        listTitle.textContent = titles[aba] || '📌 Pendências';
    }
...
function aplicarFiltrosNovo() {
    console.log('🔄 Aplicando filtros (nova visão)...');
...
function limparFiltrosNovo() {
    console.log('🧹 Limpando filtros (nova visão)...');
...
function exportarPendenciasNovo() {
    console.log('📤 Exportando pendências (nova visão)...');
    ...
    if (dados.length === 0) {
        mostrarToast('⚠️ Nenhum dado para exportar', 'warning');
        return;
    }
    ...
    link.setAttribute('download', `pendencias_${new Date().toISOString().slice(0,10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    
    mostrarToast(`✅ ${dados.length} pendências exportadas`, 'success');
}

function criarContainerNovo() {
    console.log('🔧 Criando container da nova visão...');
    ...
    containerNovo.innerHTML = `
        <div style="margin-top: 30px; border-top: 2px solid #E2E8F0; padding-top: 20px;">
            <h2 style="color: #2D3748; font-size: 18px; margin-bottom: 15px;">📊 Nova Visão - Pendências Integradas</h2>
        </div>
        ...
        <div id="loadingNovo" style="padding: 40px; text-align: center;">
            ...
            <p style="margin-top: 15px; color: #718096;">Carregando dados da nova visão...</p>
        </div>
    ...
    console.log('✅ Container da nova visão criado!');
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        // ============================================
        // INICIALIZAR A NOVA VISÃO
        // ============================================
        criarContainerNovo();
        
        // Carregar dados da nova visão
        setTimeout(() => {
...
// Exportar funções da nova visão
